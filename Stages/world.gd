...
var end_screen: Resource = preload("res://Stages/Menus/GameEndScreen/game_end_screen.tscn")
...
func show_game_end_screen(is_level_complete: bool, score: int) -> void:
	var screen: CanvasLayer = end_screen.instantiate()
	add_child(screen)
	var level_name : String = get_scene_file_path().get_file().get_basename()
	screen.setup(is_level_complete, score, level_name)

func _on_player_game_over(score: int) -> void:
	show_game_end_screen(false, score)

func _on_player_level_complete(score: int) -> void:
