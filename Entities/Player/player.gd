...
const EXPLOSION_EFFECT: Resource = preload("res://Entities/Player/Data/explosion_effect.tscn")

const ALL_PADS_BONUS: int = 500
const NO_CRASH_BONUS: int = 500
const MAX_SPEED_BONUS: int = 500
const FUEL_BONUS_MULT: float = 1.5

@export var max_speed: float = 1000.0
@export var rotation_speed: float = 125.0
@export var acceleration: float = 75.0
@export var gravity_scale: float = 0.025
@export var safe_landing_speed: float = 25.0
@export var max_safe_landing_angle: float = 15.0
@export var main_thruster_fuel_use_rate: int = 25
@export var rotation_thruster_fuel_use_rate: int = 10
@export var crash_penalty: int = -5
...
var score_popup_scene: Resource = preload("res://Objects/ScorePopup/score_popup.tscn")

var landing_checked: bool = false
var received_all_pads_bonus: bool = false
var received_max_speed_bonus: bool = false
var crash_counter: int = 0
...
func _connect_to_landing_pads() -> void:
	for pad: LandingPad in LandingPadManager.pads:
...
func apply_rotation(delta: float) -> void:
	var input_axis: float = Input.get_axis("rotate_left", "rotate_right")
...
func handle_acceleration(delta: float) -> void:
	if Input.is_action_pressed("fire_thruster") and !stats.is_fuel_empty():
		...
		var thrust: Vector2 = direction * acceleration
...
func burn_fuel(fuel_use_rate: float, delta: float) -> void:
...
func _on_ship_collider_body_entered(body: CollisionObject2D) -> void:
	if body is PhysicsBody2D:
		crash_and_reset()
	
	
func _on_landing_body_entered(body: CollisionObject2D) -> void:
	if landing_checked:
		return
	if body is LandingPad:
		handle_landing_attempt(body as LandingPad)
...
func handle_landing_attempt(landing_pad: LandingPad) -> void:
	if !is_speed_safe():
...
func crash_and_reset(message: Variant = null) -> void:
	crash_audio_stream_player.play()
	crash_counter += 1
	update_points(crash_penalty)
	play_explosion_effect()
	hud.display_message(str(message) if message != null else "Ship Crashed!", Color.RED)
	if stats.is_fuel_empty():
		handle_game_over()
	else:
...
func play_explosion_effect() -> void:
	var explosion_effect: AnimatedSprite2D = EXPLOSION_EFFECT.instantiate()
...
func award_all_pads_bonus() -> void:
